...
async function closeBidding(closeBidding) {
    ...
    // 자동차 경매 정보 변경
    const carListingRegistry = await getAssetRegistry("com.betweak.carauction.carListing");
    await carListingRegistry.update(listing);

    // 자동차 경매 정보가 SOLD이면
    if(listing.state === "SOLD") {
        // 구매자 정보 변경
        const userRegistry = await getParticipantRegistry("com.betweak.carauction.Member");
...
/** 
* Make on Offer for a carListing 
* @param {com.betweak.carauction.Offer} offer - the opper
* @transaction
*/
async function makeOffer(offer) {
    ...
    if (listing.state !== "FOR_SALE") {
        // 팔리는 중이 아니라면 에러
        throw new Error("Listing is not FOR SLAE");
    }  
    if(!listing.offers) {
        // 최초 입찰이면 빈 배열 만듦
        listing.offers = [];
